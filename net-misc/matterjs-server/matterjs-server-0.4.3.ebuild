...
# Use a clearer workdir name to avoid confusion with 'package/'
...
src_unpack() {
    ...
    # Main tarball unpacks to 'package/' → rename/move to a clear name
    mv "${WORKDIR}/package" "${S}" || die "Failed to move main package dir"

    if use server; then
        # Unpack vendor node_modules into S (creates S/node_modules if needed)
...
src_install() {
    ...
    # Install only key directories/files instead of -r . to avoid copy errors on missing files
    # Adjust list based on what your package actually contains (from ls in workdir)
    doins -r dist node_modules package.json  # core runtime files

    # Optional extras (safe – won't fail if missing)
