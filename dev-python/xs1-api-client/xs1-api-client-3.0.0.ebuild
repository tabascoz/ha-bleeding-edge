...
HOMEPAGE="https://github.com/markusressel/xs1-api-client \
	https://pypi.org/project/xs1-api-client/"
...
src_prepare() {
	...
	sed -i \
		"s/subprocess.check_output(\[\"git\", \"rev-parse\", \"--abbrev-ref\", \"HEAD\"\])/'master'/" \
		setup.py || die
	sed -i \
		"s/GIT_BRANCH = GIT_BRANCH.decode()  //" \
		setup.py || die

	# and it tries to install a 'tests' package at top level.... tsss...
	sed -i \
		"s/packages=find_packages()/packages=find_packages(exclude=['tests','tests.*'])/g" \
		setup.py || die
